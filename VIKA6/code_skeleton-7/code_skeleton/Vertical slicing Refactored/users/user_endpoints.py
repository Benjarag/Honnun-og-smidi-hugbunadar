...
def create_user(request: CreateUserDto, session_factory: Callable[..., AbstractContextManager[Session]] = Depends(get_service(Callable[..., AbstractContextManager[Session]]))):
    user = User(
        name=request.name,
        email=request.email,
